...
c
!      if(iiwrite .ne. 0) write(2,95)
95    format(/'### SVP FILE INFORMATION ###')
!      open(10,file=svp_file(1:lsvp),err=500) 
!      rewind(10)
c
      iierr=0
      nline=0
!      call star2(10,nline,2,svp_file,1,iiwrite)
!      read(10,*,end=510,err=510)
      read(svp1,*) svp_ver,svp_title
...
!         read(10,*,end=510,err=510) nsvp,ctol
...
c
!      call star2(10,nline,2,svp_file,nsvp,iiwrite)
!      read(10,*,end=510,err=510)
!      read(10,*,end=510,err=510) zsvp(1),csvp(1),rho_svp,alpha_svp
     ...
     .   'rho_svp on first line',21,iierr)
      do j=2,nsvp
!         read(10,*,end=510,err=510) zsvp(j),csvp(j)
...
!      call star2(10,nline,2,svp_file,1,iiwrite)
!      read(10,*,end=510,err=510)
!      read(10,*,end=510,err=510) nlayb
      call check_val_i(nlayb+1,1,NLMAX,eline,27,nline,'nlayb+1',
     .   7,iierr)
!      call star2(10,nline,2,svp_file,nlayb,iiwrite)
!      read(10,*,end=510,err=510)
      do j=1,nlayb
         if(svp_ver .gt. 2.e0) then
!            read(10,*,end=510,err=510) ktb(j),hb(j),((geob(j1,j2,j),
!     .         j1=1,2),j2=1,5),(bpb(j1,j),j1=1,4)
             read(blayers(j),*) ktb(j),hb(j),((geob(j1,j2,j),
     .         j1=1,2),j2=1,5),(bpb(j1,j),j1=1,4)          
         else
            read(blayers(j),*) ktb(j),hb(j),((geob(j1,j2,j),
...
c: meaning csed/cwater ratio:
      ...
      if (consprint .eq. 1) print *,'cb(1) = ',geob(1,1,1)
            endif
...
c: Nominal average gradient (since we don't know it for sure):
      ...
      if (consprint .eq. 1) print *,'hb(1) = ',hb(1)
            endif
         endif
...
!      call star2(10,nline,2,svp_file,1,iiwrite)
!      read(10,*,end=510,err=510)
!     read(10,*,end=510,err=510) nlayt
      call check_val_i(nlayt+1,1,NLMAX,eline,27,nline,'nlayt+1',
     .   7,iierr)
!      call star2(10,nline,2,svp_file,nlayt,iiwrite)
!      read(10,*,end=510,err=510)
      do j=2,nlayt+1
         if(svp_ver .gt. 2.e0) then
!           read(10,*,end=510,err=510) ktt(j),ht(j),((geot(j1,j2,j),
!    .         j1=1,2),j2=1,5),(bpt(j1,j),j1=1,4)]
             read(tlayers(j),*) ktt(j),ht(j),((geot(j1,j2,j),
     .        j1=1,2),j2=1,5),(bpt(j1,j),j1=1,4)
         else
             read(tlayers(j),*) ktt(j),ht(j),((geot(j1,j2,j),
...
c: meaning csed/cwater ratio:
            ...
            if (consprint .eq. 1)  print *,'ct(1) = ',geot(1,1,1)
            endif
...
c: Nominal average gradient (since we don't know it for sure):
      ...
      if (consprint .eq. 1) print *,'ht(1) = ',ht(1)
            endif
         endif
...
!         write(2,410) ndel,ndel+nsvp,(zdel(j),j=1,ndel)
...
c
      return
500   if (consprint .eq. 1) then 
          print *,'Error opening SVP file ',svp_file
      endif
      stop
...
200   format(a)
      goto 490
510   if (consprint .eq. 1) then
          print *,'Endo or error reading SVP file '
      endif
      stop
      end
ccc

...
!      open(46,file=opt_file(1:lopt),form='formatted',err=110)
      nline=0
      iierr=0
      iigeom=0
      
!     call star2(46,nline,2,opt_file,1,iiwrite)
!     read(46,*,end=110,err=110)
!     write(2,*) 123
!     read(opt1,*) ver_no
!     print *, ver_no
      call check_val_r4(ver_no,0.90e0,2.10e0,eline,27,nline,'ver_no',
     .   6,iierr)
      if(ver_no .lt. 1.10) then
          read(opt1,*) ver_no,iicw,iikpl,iirc,n_env
         iifmt=1
         iiparm=0
         if(iicw .eq. 3) then
            iiparm=1
...
!     call star2(46,nline,2,opt_file,1,iiwrite)
!     read(46,*,end=110,err=110)
      if(ver_no .ge. 2.01e0) then
!!        read(46,*,end=110,err=110) iirx,cphmin,cphmax,rmin,rmax,
!     .      phfac,db_cut,iiAih(1),iiAih(2),kkblm,iigbs,iidiag
      read(opt2,*) iirx,cphmin,cphmax,rmin,rmax,
     .      phfac,db_cut,iiAih(1),iiAih(2),kkblm,iigbs,iidiag
      
      elseif(ver_no .ge. 2.0e0) then
!!        read(46,*,end=110,err=110) iirx,cphmin,cphmax,rmin,rmax,
!    .      phfac,db_cut,iiAih(1),iiAih(2),iigbs,iidiag
      read(opt2,*) iirx,cphmin,cphmax,rmin,rmax,
     .         phfac,db_cut,iiAih(1),iiAih(2),iigbs,iidiag
      
      elseif(ver_no .ge. 1.6e0) then
!!        read(46,*,end=110,err=110) iirx,cphmin,cphmax,rmin,rmax,
!     .      phfac,db_cut,iiAih(1),iigbs,iidiag
      read(opt2,*) iirx,cphmin,cphmax,rmin,rmax,
     .      phfac,db_cut,iiAih(1),iigbs,iidiag
      
      elseif(ver_no .ge. 1.e0) then
!!        read(46,*,end=110,err=110) iirx,cphmin,cphmax,rmin,rmax,
!     .      phfac,db_cut,iiAih(1),iidiag
      read(opt2,*) iirx,cphmin,cphmax,rmin,rmax,
     .      phfac,db_cut,iiAih(1),iidiag
      
      elseif(ver_no .ge. 0.95e0) then
!!        read(46,*,end=110,err=110) cphmin,cphmax,rmin,rmax,phfac,
!     .      db_cut,iidiag
      read(opt2,*) cphmin,cphmax,rmin,rmax,phfac,
     .      db_cut,iidiag
         iirx=0
         iigbs=0
      else
...
!      print *, iirx,cphmin,cphmax,rmin,rmax,
!     .      phfac,db_cut,iiAih(1),iiAih(2),kkblm,iigbs,iidiag
...
!!        read(46,*,end=110,err=110) nfcw,(fcw(j),j=1,nrd(nfcw))
         read(opt3,*) nfcw,(fcw(j),j=1,nrd(nfcw))
!        call star2(46,nline,2,opt_file,1,iiwrite)
!        read(46,*,end=110,err=110)
         if(ver_no .ge. 1.e0) then
            
            read(opt4,*)  iitl,iimf,iisig,iimt,iidc,
...
c
   ...
   !        call star2(46,nline,2,opt_file,1,iiwrite)
   !        read(46,*,end=110,err=110)
            read(opt7,*) nr_tsp,r1_tsp,r2_tsp,
...
!        read(46,*,end=110,err=110)
         if(ver_no .ge. 1.01) then
            read(opt7,*) fsbb,Tw,fmin,fmax,iifft,
...
c: Make sure we write out nparm lines to output file:
         read(opt9,*) nrun,nparm
         backspace(46)
         backspace(46)
         backspace(2)
!        call star2(46,nline,2,opt_file,nparm,iiwrite)
!        read(46,*,end=110,err=110)
         if(ver_no .lt. 1.5) then
            read(opt9,*) nrun,nparm,(kvar(1,j),
     .         kvar(2,j),kvar(3,j),kvar(4,j),xvar(1,j),xvar(2,j),
     .         j=1,min(NPMAX,iabs(nparm)))
         else
            read(opt9,*) nrun,nparm,rseed,(kvar(1,j),
     .         kvar(2,j),kvar(3,j),kvar(4,j),xvar(1,j),xvar(2,j),
     .         j=1,min(NPMAX,iabs(nparm)))
         endif
         call check_val_i(nrun,-1000,1000,eline,27,nline,
     .      'nrun',4,iierr)
         call check_val_i(nparm,1,NPMAX,eline,27,nline,
     .      'nparm',5,iierr)
      else
!        call star2(46,nline,2,opt_file,0,iiwrite)
      endif
...
!        call star2(46,nline,2,opt_file,1,iiwrite)
         nzs=1
!        read(46,*,end=110,err=110)
         read(opt10,*) zsrc(1),nseg,geom_file
         call lname64(geom_file,lgeom)
         call check_val_i(nseg,1,NSEGMAX,eline,27,nline,
     .      'nseg',4,iierr)
         call src_track(nline,eline,iierr)
      else
!        call star2(46,nline,2,opt_file,0,iiwrite)
!        call star2(46,nline,2,opt_file,0,iiwrite)
      endif
...
!        call star2(46,nline,2,opt_file,1,iiwrite)
!        read(46,*,end=110,err=110)
         read(opt12,*) fkpl,iivar,iiform,xkhr1,xkhr2,
...
!        read(46,*,end=110,err=110)
         if(ver_no .lt. 2.1) then
            read(opt13,*) freq1,freq2,nfreq,iilog,
     .         th1,th2,nang
...
!        call star2(46,nline,2,opt_file,1,iiwrite)
!        read(46,*,end=110,err=110)
         read(opt14,*) freq1,freq2,fsrc,nfft,
     .      th1,th2,nang
         call check_val_r4(freq1,1.e-3,freq2,eline,27,nline,
     .      'freq1',5,iierr)
...
c
     ...
     .      'for error(s).'
      ...
      return
c
110   if (consprint .eq. 1) then
        print *,'Error opening or reading option file ',opt_file(1:lopt)
...
c
      return
99    if (consprint .eq. 1) then
          print *,'Error opening array geometry file ',geom_file
      endif
      stop
      end
ccc
...
!            write(nfout,110) chfs(1:lchfs)
            if(necho .eq. 13) then
                if (consprint .eq. 1) print *, j, chfs(1:lchfs)
            endif
         enddo
         do j=1,necho
            backspace(nf)
